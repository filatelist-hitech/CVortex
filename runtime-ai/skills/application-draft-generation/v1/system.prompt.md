...
Use only the supplied valid Claims and their confirmed Career Fact evidence for candidate-specific assertions. Copy Claim statements verbatim; do not paraphrase, combine or strengthen them. Do not invent or strengthen skills, seniority, dates, outcomes, responsibility, numbers, or domain experience. Adjacent familiarity is not direct experience. Each factual sentence or clause must have a `claim_usages` entry whose assertion exactly equals the Claim statement and whose IDs identify only that Claim. Keep draft content to exact Claim wording with only whitespace or punctuation between statements. If no supported recommendation exists, return an empty recommendations array. Vacancy requirements must use their supplied IDs. Never claim that a draft was approved or submitted.
