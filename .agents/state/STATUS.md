...
### MCP Gateway Foundation — COMPLETE locally, 2026-09-26

The bounded, explicitly user-authorized `feature-mcp-gateway-foundation` task is implemented in a separate branch based on `stage`. The original unrelated dirty checkout was preserved. Inbound `laravel/mcp` Streamable HTTP is disabled by default and exposes only owned Vacancy metadata, bounded current application context and Truth Guard-validated cover draft submission. Passport bearer `mcp:use`, active-user enforcement, owner context/RLS, rate limits and metadata-only audit logs protect the boundary. MCP `PENDING_REVIEW` maps to persisted M1.4 `DRAFT`; there is no MCP approval, fact confirmation or send tool. The existing Responses API provider and `OPENAI_API_KEY` path remain intact. No Employer Memory/Consistency service was invented before M4.

Local validation PASS: full backend 206 tests / 1274 assertions (six PostgreSQL-only skips), Pint, Larastan zero errors, Compose config; isolated PostgreSQL fresh migration, runtime-role Application security 2 tests / 45 assertions, own/foreign MCP Inspector read/write proof, legacy initialize/current discover, and six-migration rollback/re-up preserving a preexisting User. The isolated runtime role has neither superuser nor BYPASSRLS. The Inspector write used a temporary test-only fake LLM provider; production Truth Guard still requires the configured `LlmProvider` and fails closed if unavailable. See `docs/10-Operations/MCP-Gateway-Validation.md` for exact evidence.

Finalization connected the Passport consent view, restricted DCR to the currently supported callback-ID-specific ChatGPT redirect, and verified current official plan/read/write, OAuth and tunnel documentation. Focused MCP tests passed (7 tests / 65 assertions), Access Core passed (35 / 167), Pint on changed backend files and Larastan passed, disabled/enabled route registration and Compose config passed, and `git diff --check` passed. These checks do not replace the earlier full backend/PostgreSQL evidence. `CHATGPT CONNECTION = NOT_VALIDATED`: this account/workspace, public/tunnel reachability, complete OAuth flow and `resource` audience enforcement were not tested. Secure MCP Tunnel needs Platform tunnel permissions, a tunnel ID and runtime API key; billing/funded-credit requirements remain UNKNOWN. Preview 0.1 remains INCOMPLETE, full M1.4 acceptance remains unvalidated, and M2 has not started.

PR #33 test bootstrap was corrected after its first `m0-quality` run exposed the default-disabled Compose environment in MCP tests. The PHPUnit-only override enables the gateway during tests without changing runtime defaults. The focused MCP suite passed (7 / 65) and the full backend suite passed with external `MCP_ENABLED=false` (207 / 1278, six PostgreSQL-only skips); Pint, Larastan and disabled/enabled route sanity passed. This does not validate a ChatGPT account connection or complete Preview 0.1.
