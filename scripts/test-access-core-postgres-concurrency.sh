...
same_email_reusable=$("${compose[@]}" exec -T -e DB_DATABASE="$database" postgres psql -U "$pg_user" -d "$database" -Atqc "SELECT count(*) FROM invitations WHERE id IN ('$same_email_id_one', '$same_email_id_two') AND uses = 0 AND consumed_at IS NULL AND revoked_at IS NULL")
...
docker compose exec -T -e DB_DATABASE="$disable_database" backend php artisan migrate --database=pgsql_admin --force >/dev/null
