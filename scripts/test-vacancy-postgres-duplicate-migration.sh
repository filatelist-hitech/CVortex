...
('00000000000000000000000018', '00000000000000000000000001', 'PASTED_TEXT', 'https://jobs.example.test/matching-failed', 'FAILED', 'ANALYSIS_ERROR', '2024-05-01 00:00:00+00', now());
...
('00000000000000000000000030', '00000000000000000000000001', '00000000000000000000000018', 1, 'matching failed after extraction', 'https://jobs.example.test/matching-failed', repeat('a', 64), '2024-05-01 00:00:00+00', '2024-05-01 00:00:00+00', now());
...
('00000000000000000000000038', '00000000000000000000000001', '00000000000000000000000030', 'vacancy_requirement_extraction', 'vacancy.requirement-extraction', '1.0.0', '1.0.0', 'default', 'COMPLETED', 'PASS', NULL, now(), now());
SQL
...
BEGIN
    IF (SELECT count(*) FROM vacancies) <> 5 THEN failures := array_append(failures, 'duplicate vacancies not converged or matching failure row lost'); END IF;
    IF (SELECT count(*) FROM vacancy_snapshots) <> 10 THEN failures := array_append(failures, 'snapshots not preserved'); END IF;
    IF (SELECT count(*) FROM vacancy_analyses) <> 5 THEN failures := array_append(failures, 'analyses not preserved'); END IF;
    IF (SELECT count(*) FROM vacancy_llm_runs) <> 3 THEN failures := array_append(failures, 'LLM runs not preserved'); END IF;
    ...
    IF (SELECT analysis_status FROM vacancies WHERE source_url = 'https://jobs.example.test/matching-failed') <> 'FAILED' THEN failures := array_append(failures, 'matching failure was cleared into unrecoverable pending state'); END IF;
    IF (SELECT error_code FROM vacancies WHERE source_url = 'https://jobs.example.test/matching-failed') <> 'ANALYSIS_ERROR' THEN failures := array_append(failures, 'matching failure code was cleared'); END IF;
...
DO $$
BEGIN
    IF (SELECT count(*) FROM vacancies) <> 5 OR (SELECT count(*) FROM vacancy_snapshots) <> 10 THEN RAISE EXCEPTION 'rollback/re-up lost vacancy data'; END IF;
    ...
    IF (SELECT analysis_status FROM vacancies WHERE source_url = 'https://jobs.example.test/matching-failed') <> 'FAILED' OR (SELECT error_code FROM vacancies WHERE source_url = 'https://jobs.example.test/matching-failed') <> 'ANALYSIS_ERROR' THEN RAISE EXCEPTION 'rollback/re-up changed matching failure state'; END IF;
END;
$$;
SQL
