...
"${compose[@]}" exec -T -e DB_DATABASE="$database" backend php artisan migrate --database=pgsql_admin --force \
...
"${compose[@]}" exec -T -e DB_DATABASE="$database" backend php artisan migrate:rollback --database=pgsql_admin --step=3 --force >/dev/null
