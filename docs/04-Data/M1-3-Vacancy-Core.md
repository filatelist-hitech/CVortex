...
Historical residence intervals such as `Based in London from 2018 to 2020` do not prove a candidate's current location; a separate explicit current location statement can still match. Subject-bound passive negation such as `Kubernetes was not used in production` cannot count as positive technical evidence. Confirmed native language proficiency satisfies a fluent requirement, while fluent evidence does not satisfy a native requirement; other non-CEFR descriptors retain exact matching.
