...
Exact source/candidate comparisons and supported structured values are deterministic. Provider labels for technical/domain requirements must identify the cue-bound source-derived subject, including active wording such as `role requires Kubernetes`, not merely a generic word such as `experience` or an unrelated role descriptor. Token/skill matches are boundary-aware: `Go` can match `Go backend` but not `Google`; compound technical and domain subjects must occur as one contiguous phrase in candidate evidence. Location uses canonical equality, so `US` cannot match `Russia`. A `located in` or residence cue classifies only the named place it governs; an unrelated subject such as `distributed systems` is not a location because a later phrase mentions where the systems operate. Place capture stops before a second predicate such as `and working remotely`, while retaining compound place names. Work format requires an arrangement label and wording such as `remote work`, `fully remote`, `hybrid role`, `on-site role` or `office-based`; remote-access products, collaboration platforms, APIs and hybrid architecture do not establish a work format. Historical statements such as `I worked remotely in 2022` and time-bounded remote employee roles such as `Remote employee at Acme from 2020 to 2022` do not constrain a current work-format requirement. Current work-format statements and preferences can carry more than one accepted arrangement, and any explicitly acceptable format can satisfy the matching requirement. Candidate inability evidence such as `cannot work remotely` is never affirmative evidence for a structured work-format requirement. A stale `RUNNING` analysis exposes recovery and can be reclaimed through the existing reanalysis endpoint. Language evidence must tie a proficiency, fluency or level construction to the named language, including bounded forms such as `English at B2 level`, `English level B2` and `B2 English`; this grammar applies to qualified language names beyond the built-in canonical aliases. An English-speaking team or English documentation remains incidental. Candidate evidence must support the named language and qualification. A CEFR-qualified language requirement matches confirmed evidence for the same language at an equal or higher CEFR level (A1–C2); other descriptors such as `native`, `fluent` and `upper-intermediate` retain exact handling without inferred equivalence. Salary comparison retains lower, upper, exact and range semantics: overlapping intervals, including a shared endpoint, are compatible; only an explicit non-overlapping interval in the same currency and comparable pay period is a blocker. Missing, mixed-currency or incomparable-period values remain unknown. Unquantified experience can use exact confirmed commercial/direct evidence after removing source-only requirement-strength framing; commercial/professional/production wording in the source makes the requirement `EXPERIENCE` even when provider output calls the subject a technical skill. Source constructions such as `experience in Kubernetes` and `Kubernetes experience` also derive `EXPERIENCE`, and `no longer required` negates that subject before importance inference. Duration comparison requires a source-supported number and explicit unit tied to experience, plus a candidate duration tied to the same subject. Canonical values use `years:<number>` or `months:<number>`; years/months convert only when both sides state units, and sentence framing such as `we require` is not part of the subject. Ambiguous multi-duration or unrelated duration wording remains unknown. All relevant values are considered before declaring incompatibility. Conservative adjacency families can identify weak related evidence, but adjacency remains a gap counted according to requirement importance and never upgrades familiarity into direct/commercial experience. One or two unsupported mandatory requirements, including adjacent evidence, cap the recommendation at `MAYBE`; three or more produce `LOW_PRIORITY`. Missing candidate values for mandatory structured requirements remain unknown and cap the recommendation at `MAYBE`.

Historical residence intervals such as `Based in London from 2018 to 2020` do not prove a candidate's current location; a separate explicit current location statement can still match. Ongoing residence qualifiers such as `since 2018` or `from 2018 to present` do not change the parsed place. Subject-bound passive negation such as `Kubernetes was not used in production` or `Kubernetes wasn't used in production` cannot count as positive technical evidence; “not only used” remains affirmative. Confirmed native language proficiency satisfies a fluent requirement, while fluent evidence does not satisfy a native requirement; other non-CEFR descriptors retain exact matching.
